...
/**
 * 列車モデルの実際のドア位置 (車体ローカル座標) を求める。
 * 乗客 NPC を停止位置目標ではなく「本当のドア」に吸い付かせるために使う。
 */
public final class TrainDoorLocator {
    ...
    private static List<float[]> compute(String id) {
        try {
            ...
            if (!doors.isEmpty()) {
                ...
                // スクリプト制御でドア定義が無い車両 等): モデルから「ドア」グループを名前で探す。
    ...
    /**
     * JSON にドア定義が無い (スクリプト制御) 車両向け: モデルから名前に「door」を含む外板の
     * スライドドアグループを探し、開口ごとにまとめて位置を返す。
     * <ol>
     * 名前に door を含み、ランプ/窓/取っ手/内装等の語を含まないグループの重心を集める
     * 車体側面 (|x| が大きい方) のものだけ残す (内装扉・中央通路扉を除外)
     * 近いもの同士 (同じ扉の複数リーフ o/i・a/b) を 1 つにまとめる
     * </ol>
     */
    private static List<float[]> scanDoorsFromModel(PolygonModel model, Vec3 off, float scale) {
        // ① door を含む非除外グループの重心を車体ローカルで集める。
