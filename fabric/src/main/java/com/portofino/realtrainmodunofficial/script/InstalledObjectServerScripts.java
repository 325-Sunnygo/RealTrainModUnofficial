 ...
 * 設置物の定義 JSON に "serverScriptPath": "scripts/xxx.js" があると、本家は
 * サーバー側で毎 tick onUpdate(entity, scriptExecuter) を呼ぶ。
 */
public final class InstalledObjectServerScripts {
     ...
     * importPackage では解決できない (1.21 に該当パッケージが無い) ものだけ。
     */
    ...
    public static void tick(ServerLevel level, InstalledObjectBlockEntity be) {
        ...
        try {
            ...
            // 本家 ScriptUtil.doScriptIgnoreError と同じく、失敗しても止めない。
            // 以前はここで FAILED.add(id) して定義ごと永久停止させていたため、
            // 1 個の設置物が 1 tick こけただけで同じ種類の設置物が全部動かなくなっていた。
            if (LOGGED.add(id)) {
