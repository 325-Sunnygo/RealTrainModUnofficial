 ...
 *
 * <p>★以前は選択範囲をエンティティ ({@code EditorEntity}) として世界に置いていたが、
 * 1.21 では
 * <ul>
 *   <li>見えているチャンクセクションのエンティティしか描画に集められない</li>
 *   <li>離れると <b>クライアント側の追跡が切れて消える</b></li>
 *   <li>チャンクが退避するとサーバ側からも引けなくなる</li>
 * </ul>
 * のため「選択がすぐ消える」状態になっていた。
 *
 * <p>MCTEU は選択をエンティティにせず、クライアントの静的な {@code pos1/pos2} に持ち、
 * 操作のたびにサーバへ送っている。それに合わせ、ここでも<b>プレイヤーごとの素のデータ</b>にする。
 * ワールドに実体が無いので、どこへ飛んでも消えない。
 */
