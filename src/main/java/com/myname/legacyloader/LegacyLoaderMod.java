...
public class LegacyLoaderMod {
    ...
    public LegacyLoaderMod(IEventBus modBus) {
        ...
        //クライアント専用イベント (モデル差し替え/TESR/BEWLR) は別クラス LegacyLoaderClient に分離。
        //本体クラスに client 専用メソッド (BlockEntityRenderer 等を参照) を置くと、専用サーバー起動時に
        //クラス解決で NoClassDefFoundError になり落ちるため。Dist.CLIENT のときだけ読み込む。
        if (FMLEnvironment.dist == Dist.CLIENT) {
            LegacyLoaderClient.register(modBus);
        }
